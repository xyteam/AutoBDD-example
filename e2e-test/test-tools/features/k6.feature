...
Feature: k6 performance test
  ...
  Scenario: run the k6 performance test
    When I run this command "$PROJECTRUNPATH/e2e-test/test-tools/support/provision-k6.sh && cd $PROJECTRUNPATH/k6-test && $HOME/.local/bin/k6 run performance-test.js 2>&1"
