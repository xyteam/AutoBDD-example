...
Feature: Sample Snippets test part 1
    ...
    Scenario: open URL
        Given the page url is not "http://localhost:8082/snippets.html"
        And   I open the url "http://localhost:8082/snippets.html"
        Then  I expect the full URL to be "http://localhost:8082/snippets.html"
        And   I expect the full URL to not be "http://google.com"

    Scenario: open sub page of weburl
        Given the page url is not "http://localhost:8082/two.html"
        And   I open the url "http://localhost:8082/snippets.html"
        Then  I expect the full URL to be "http://localhost:8082/snippets.html"
        And   I expect the full URL to not be "http://google.com"
    ...
    Scenario: wait for element to be displayed
        ...
        Then  I wait on the element ".lateElem" for 5000ms to be displayed

    Scenario: wait for element using default wait time
        Given I open the url "http://localhost:8082/snippets.html"
        And   there is no element ".lateElem" on the page
        Then  I wait on the element ".lateElem" to be displayed
